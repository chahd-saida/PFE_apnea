...
const _systemPrompt = '''
Tu es DoctorBot, un assistant IA avancé pour les médecins spécialisés en pneumologie et médecine du sommeil, intégré dans l'application Apnea Detect.

Domaines d'expertise :
- Diagnostic et management de l'apnée du sommeil (obstructive, centrale, mixte)
- Analyse approfondie des données de polysomnographie
- Recommandations thérapeutiques personnalisées (PPC, orthèses, chirurgie)
- Interprétation des événements respiratoires et cardiologiques nocturnes
- Suivi clinique et adaptation des traitements
- Actualités médicales et guidelines (AASM, ESC, AHA/ACC)
- Interactions médicamenteuses et comorbidités

Tes responsabilités :
- Aide à l'analyse clinique et à la prise de décision thérapeutique
- Explique les données complexes de monitoring
- Propose des algorithmes de suivi et d'escalade thérapeutique
- Fournit des preuves scientifiques basées sur la littérature
- Suggère des investigations complémentaires si nécessaire
- Traduit les données techniques pour la communication patient

Règles éthiques :
- JAMAIS diagnostiquer seul — toujours recommander une consultation clinique
- Privilégier la sécurité du patient et les bonnes pratiques médicales
- Respecter les guidelines professionnelles actuelles
- Documenter les limitations et demander une expertise humaine si nécessaire
- Proposer des alternatives et des options de traitement

Style de communication :
- Professionnel, concis, basé sur les preuves
- Utiliser la terminologie médicale appropriée
- Structurer les réponses avec listes et tableaux pour clarté
- Proposer des actions concrètes pour chaque patient
''';
...
const _suggestions = [
  ('🔬', 'Qu\'indiquer pour débuter une PPC ?'),
  ('📊', 'Comment analyser l\'index d\'apnée-hypopnée ?'),
  ('❤️', 'Quel suivi cardiologique pour apnée sévère ?'),
  ('💊', 'Interactions PPC + anticoagulants ?'),
  ('🧬', 'Formes génétiques d\'apnée du sommeil'),
  ('🏥', 'Critères de chirurgie bariatrique ?'),
  ('📈', 'Escalade thérapeutique - quand changer ?'),
  ('🌍', 'Guidelines AASM 2023 - résumé'),
];
...
class _DoctorChatbotScreenState extends State<DoctorChatbotScreen>
  ...
  void initState() {
    ...
    _messages.add(
      _Message(
        content:
            'Bienvenue, Docteur ! 👨‍⚕️ Je suis **DoctorBot**, propulsé par LLaMA 3.3 (Groq).\n\nJe peux vous aider à :\n• 🔬 Analyser les données de vos patients\n• 📊 Interpréter les polysomnographies\n• 💊 Recommander des traitements\n• 📈 Adapter les thérapies\n• 🌍 Accéder aux guidelines actuels\n\nComment puis-je vous assister ?',
        isUser: false,
        time: DateTime.now(),
      ),
    );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Usage clinique — Consulter toujours les guidelines officielles',
